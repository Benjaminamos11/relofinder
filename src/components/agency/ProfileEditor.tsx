...
import { Save, Upload, MapPin, Globe, Loader2 } from 'lucide-react';
    ...
    return (
        <form onSubmit={handleSubmit} className="space-y-8 max-w-7xl mx-auto pb-40">
            ...
            <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
                ...
                {/* LEFT COLUMN: Core Identity & Contact (2/3 width on huge screens, full on mobile) */}
                        ...
                        <div className="p-6 space-y-6">
                            <div className="flex flex-col md:flex-row gap-6">
                                <div className="shrink-0">
                                    <label className="block text-xs font-bold uppercase tracking-wide text-slate-500 mb-2">Logo</label>
                                    <div className="w-32 h-32 bg-slate-100 rounded-lg border-2 border-dashed border-slate-300 flex flex-col items-center justify-center text-slate-400 hover:border-slate-400 hover:bg-slate-50 transition-colors cursor-pointer group relative overflow-hidden">
                                        {formData.logo ? (
                                        ...
                                        <input type="text" name="logo" value={formData.logo} onChange={handleChange} className="absolute inset-0 opacity-0 cursor-pointer" title="Paste URL here for now" />
                                    </div>
                                </div>
                            ...
                            <div>
                                ...
                                <textarea name="bio" value={formData.bio} readOnly className="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-lg text-slate-500 cursor-not-allowed text-sm" rows={3} />
                            </div>
                        </div>
                    </div>
                        ...
                        <div className="p-6">
                            <div className="grid md:grid-cols-2 gap-6">
                                ...
                                <div>
                                    ...
                                    <input type="tel" name="phone_number" value={formData.phone_number} onChange={handleChange} className="w-full px-3 py-2 border border-slate-200 rounded-lg" />
                                </div>
                                <div>
                                    <label className="block text-sm font-medium text-slate-700 mb-1">General Email</label>
                                    <input type="email" name="contact_email" value={formData.contact_email} onChange={handleChange} className="w-full px-3 py-2 border border-slate-200 rounded-lg" />
                                </div>
                                <div className="md:col-span-2">
                                    ...
                                    <div className="flex">
                                        ...
                                        <input type="text" name="meeting_url" value={formData.meeting_url ? formData.meeting_url.replace('https://', '') : ''} onChange={(e) => setFormData({ ...formData, meeting_url: 'https://' + e.target.value.replace('https://', '') })} className="flex-1 px-3 py-2 border border-slate-200 rounded-r-lg" placeholder="calendly.com/..." />
                                    </div>
                                </div>
                            </div>
                        </div>
                ...
                {/* RIGHT COLUMN: Operations & Manager (1/3 width) */}
                <div className="space-y-6">
                    ...
                    <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
                        ...
                        <div className="p-6 space-y-6">
                            <div>
                                <label className="block text-xs font-bold uppercase tracking-wide text-slate-500 mb-3">Regions</label>
